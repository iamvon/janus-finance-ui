...
import {CogIcon, ExclamationCircleIcon, InformationCircleIcon, SwitchVerticalIcon,} from '@heroicons/react/outline'
import SwapTokenInfo, {numberFormatter} from './SwapTokenInfo'
import {useViewport} from '../../hook/useViewport'
import {getTokenAccountsByOwnerWithWrappedSol, nativeToUi, zeroKey,} from '@blockworks-foundation/mango-client'
import useLocalStorageState from '../../hook/useLocalStorageState'
import _, {sortBy, sum} from 'lodash'
...
const JupiterForm = ({inputMintAddress, outputMintAddress, tokenDetail}) => {
    ...
    const extensions = tokenDetail.extensions
    const isStared = !!wishlist.includes(tokenDetail.address)
                ...
                <div className="flex flex-col md:flex-row md:space-x-6">
                    ...
                    <div className="w-full md:w-1/2 lg:w-1/3">
                        <div className={'token-info'}>
                            <div className={'token-name'}>{tokenDetail.name}</div>
                            <div className={'token-buttons'}>
                                <button className={'star-btn flex items-center'} onClick={() => onStarClick(tokenDetail, isStared)}>
                                    {
                                        !isStared ? (
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                 xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M21.2009 3.92182C20.063 2.6987 18.4684 2.00277 16.7978 2.00019C15.1259 2.00212 13.5297 2.69769 12.39 3.92103L12.0016 4.33147L11.6132 3.92103C9.35176 1.48711 5.54536 1.34733 3.11148 3.60877C3.00363 3.70902 2.89947 3.81314 2.79922 3.92103C0.40026 6.5086 0.40026 10.5076 2.79922 13.0951L11.4308 22.1977C11.7294 22.513 12.2271 22.5265 12.5423 22.2278C12.5527 22.2181 12.5627 22.208 12.5725 22.1977L21.2009 13.0951C23.5997 10.5079 23.5997 6.50911 21.2009 3.92182ZM20.0632 12.0125H20.0624L12.0016 20.5143L3.9401 12.0125C2.10743 10.0353 2.10743 6.98009 3.9401 5.00294C5.60438 3.19912 8.41589 3.08601 10.2197 4.75029C10.3073 4.8311 10.3916 4.91536 10.4724 5.00294L11.4308 6.01409C11.7467 6.32792 12.2566 6.32792 12.5725 6.01409L13.5309 5.00373C15.1952 3.1999 18.0067 3.0868 19.8105 4.75108C19.8981 4.83188 19.9824 4.91615 20.0632 5.00373C21.9118 6.984 21.9253 10.0448 20.0632 12.0125Z"
                                                    fill="#00FFA3"/>
                                            </svg>
                                        ) : (
                                            <FontAwesomeIcon icon={fullHeart} className={'star-btn-icon'}/>
                                        )
                                    }
                                </button>
                                {
                                    extensions.twitter && (
                                        <a className={'twitter-btn'} href={extensions.twitter} target={'_blank'}
                                           rel="noreferrer">
                                ...
                                {
                                    extensions.discord && (
                                        <a className={'discord-btn'} href={extensions.discord} target={'_blank'}
                                           rel="noreferrer">
                                ...
                                {/*{*/}
                                {/*    extensions.facebook && (*/}
                                {/*        <a className={'facebook-btn'} href={extensions.facebook} target={'_blank'}*/}
                                {/*           rel="noreferrer">*/}
                                ...
                                {/*{*/}
                                {/*    extensions.reddit && (*/}
                                {/*        <a className={'reddit-btn'} href={extensions.reddit} target={'_blank'}*/}
                                {/*           rel="noreferrer">*/}
                                {/*            <FontAwesomeIcon icon={faReddit} className={CN("text-base")}/>*/}
                                {/*            <span className={'button-text'}>Reddit</span>*/}
                                {/*        </a>*/}
                                {/*    )*/}
                                {/*}*/}
                                {
                                    extensions.website && (
                                        <a className={'website-btn'} href={extensions.website} target={'_blank'}
                                           rel="noreferrer">
                            ...
                            <div className="bg-th-bkg-2 rounded-lg p-6 SwapBox">
                                ...
                                <div className="flex justify-center my-2">
                                    <button onClick={handleSwitchMints} className="IconSwap">
                                        <SwitchVerticalIcon
                                            className="default-transition h-8 w-8 rounded-full p-1.5 bg-th-bkg-4 text-th-fgd-1 hover:text-th-primary"/>
                                    </button>
                                </div>
                    ...
                    Balance: {outputWalletBalance()}
                                ...
                                {routes?.length && selectedRoute ? (
                                    <div className="mt-8 text-th-fgd-3 text-xs">
                                        <div
                                            className="border border-[#34434F] border-th-bkg-4 mb-4 pb-4 px-3 pt-4 relative rounded-md">
                                            {selectedRoute === routes[0] ? (
                                                <div
                                                    className="absolute bg-[#00FFA3] text-[#333] font-bold px-2 py-1 rounded-sm text-th-bkg-1 text-xs -top-3">
                                                    Best Swap
                                                </div>
                                            ) : null}
                                                ...
                                                <div>
                                                    ...
                                                    tokenPrices?.inputTokenPrice ? (
                                                        ...
                                                        >
                                                            ...
                                                            %{' '}
                                                            <span
                                                                className="text-th-fgd-4">{`${((formValue?.amount / outAmountUi -
                                                                    tokenPrices?.outputTokenPrice /
                                                                    tokenPrices?.inputTokenPrice) /
                                                                (formValue?.amount / outAmountUi)) *
                                                            100 <=
                                            ...
                                            Fees paid to {info.marketMeta?.amm?.label}
                                                        ...
                                                        >
                                                            <InformationCircleIcon
                                                                className="cursor-help h-3.5 ml-1.5 w-3.5 text-th-primary"/>
                                                        </Tooltip>
                                                    </div>
                                                </div>
                                            ) : (
                                                            ...
                                                            <div className="flex items-center">
                                                                ...
                                                                <Tooltip
                                                                    content={
                                                                        <>
                                                                            {depositAndFee?.ataDepositLength ? (
                                                                                <div>You need to have an Associated
                                                                                    Token Account.</div>
                                                                            ) : null}
                                                            ...
                                                            <div className="flex flex-col text-left">
                                                                <div className="whitespace-nowrap overflow-ellipsis">
                                                                    {route.marketInfos.map((info, index) => {
                                                                        ...
                                                                        return (
                                                                            <span
                                                                                key={index}>{`${info.marketMeta.amm.label
                                                                            } ${includeSeparator ? 'x ' : ''}`}</span>
                                                                        )
                                                                    })}
                                                                </div>
